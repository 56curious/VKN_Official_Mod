...
[_debris_branches,_rain_fog] remoteExec ["VKN_fnc_monsoon_effect",0,true];
if(_rain_drop) then {[] remoteExec ["VKN_fnc_monsoon_rain_drop",0,true]};
if(_thunder_steroids) then {
		_rand_pl = [] call VKN_monsoon_alias_hunt;
...
[] spawn {
	...
	while {al_monsoon_om} do
	{
		...
		[poz_f_1,_tip_tunet,_numar_sclipiri,_sound_tunet,_storm_clouds] remoteExec ["VKN_fnc_monsoon_thunder_storm"];
		sleep _delayth2;
	};
};
};
...
if (_effect_on_objects) then {
	...
	// interval object blow
		...
		[] spawn {
			...
			[] remoteExec ["VKN_fnc_monsoon_camera_shake"];
		};
