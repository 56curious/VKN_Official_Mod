...
Author:
	...
	Quiksilver (contact: armacombatgroup@gmail.com)   - Modified by Curious
...
_QS_ST_map_enableUnitIcons = profileNamespace getVariable ["VKN_ShowMapIcons_Var", TRUE];							// BOOL. TRUE to enable MAP unit/vehicle Icons. Default TRUE.
_QS_ST_gps_enableUnitIcons = profileNamespace getVariable ["VKN_ShowGPSIcons_Var", TRUE];							// BOOL. TRUE to enable GPS unit/vehicle Icons. Default TRUE.
_QS_ST_enableGroupIcons = profileNamespace getVariable ["VKN_ShowGroupIcons_Var", TRUE];								// BOOL. TRUE to enable Map+GPS+HUD GROUP Icons. Default TRUE.
...
_QS_ST_iconColor_EAST = [0.867,0.137,0.137,0.65];							// ARRAY (NUMBER). RGBA color code.	Default [0.5,0,0,0.65];
_QS_ST_iconColor_WEST = [0.137,0.525,0.867,0.65];						// ARRAY (NUMBER). RGBA color code. Default [0,0.3,0.6,0.65];
_QS_ST_iconColor_RESISTANCE = [0.137,0.867,0.306,0.65];					// ARRAY (NUMBER). RGBA color code. Default [0,0.5,0,0.65];
_QS_ST_iconColor_CIVILIAN = [0.596,0.137,0.867,0.65];					// ARRAY (NUMBER). RGBA color code. Default [0.4,0,0.5,0.65];
_QS_ST_iconColor_UNKNOWN = [0.867,0.867,0.137,0.65];						// ARRAY (NUMBER). RGBA color code. Default [0.7,0.6,0,0.5];

//================= MEDICAL

_QS_ST_showMedicalWounded = profileNamespace getVariable ["VKN_ShowMedicalWounded_Var", TRUE];								// BOOL. TRUE to show wounded on the map and GPS. FALSE to not show wounded on the map with this script. Default TRUE.
...
_QS_ST_showFactionOnly = profileNamespace getVariable ["VKN_ShowFactionOnly_Var", true];									// BOOL. will override ST_showFriendlySides TRUE. If TRUE then will only show players faction. If FALSE then can show friendly factions. Default FALSE.
...
_QS_ST_iconMapText = profileNamespace getVariable ["VKN_ShowIconMapText_Var", true];										// BOOL. TRUE to show unit/vehicle icon text on the map. FALSE to only show the icon and NO text (name/class). Default TRUE.
_QS_ST_showMOS = profileNamespace getVariable ["VKN_ShowMOS_Var", true];											// BOOL. TRUE = show Military Occupational Specialty text(unit/vehicle class/role display name), FALSE = disable and only show icons and names. Default FALSE.
...
_QS_ST_GPSshowNames = profileNamespace getVariable ["VKN_ShowGPSNames_Var", false];									// BOOL. TRUE to show unit names on the GPS display. Default FALSE.
_QS_ST_GPSshowGroupOnly = profileNamespace getVariable ["VKN_ShowGPSGroupOnly_Var", false];								// BOOL. TRUE to show only group members on the GPS display. Default TRUE.
...
_QS_ST_showGroupMapIcons = profileNamespace getVariable ["VKN_ShowGroupMapIcons_Var", true];								// BOOL. Group icons displayed on map. Default TRUE.
_QS_ST_showGroupHudIcons = profileNamespace getVariable ["VKN_Show3DGroupIcons_Var", false];								// BOOL. Group icons displayed on player 3D HUD. Default FALSE.
...
QS_ST_STR_text1 = 'Click to show group info...';				// STRING. Text shown when a player passes Mouse over Group leader (only if _QS_ST_groupInteractiveIcons = TRUE;)
...
_value = profileNamespace getVariable ["VKN_MapIconValue_var", true];
if (_value) then {
